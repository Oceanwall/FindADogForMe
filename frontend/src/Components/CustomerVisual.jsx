...
class CustomerVisual extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      console.log(
        "Loading catastrophe_world data through API failed due to their API's CORS error, falling back to data file."
      );
    ...
    for (const key of Object.keys(state_disaster_pairings)) {
      ...
      if (num <= 4) state_disaster_pairings[key]["fillKey"] = "0-4";
      else if (num <= 9) state_disaster_pairings[key]["fillKey"] = "5-9";
      else if (num <= 14) state_disaster_pairings[key]["fillKey"] = "10-14";
      else if (num <= 19) state_disaster_pairings[key]["fillKey"] = "15-19";
      else if (num <= 29) state_disaster_pairings[key]["fillKey"] = "20-29";
      else if (num <= 39) state_disaster_pairings[key]["fillKey"] = "30-39";
      else if (num <= 49) state_disaster_pairings[key]["fillKey"] = "40-49";
      else if (num <= 59) state_disaster_pairings[key]["fillKey"] = "50-59";
      else if (num <= 69) state_disaster_pairings[key]["fillKey"] = "60-69";
      else if (num <= 79) state_disaster_pairings[key]["fillKey"] = "70-79";
      else if (num <= 89) state_disaster_pairings[key]["fillKey"] = "80-89";
      else state_disaster_pairings[key]["fillKey"] = "defaultFill";
    }
    ...
    for (const key of Object.keys(state_organization_pairings)) {
      ...
      if (num <= 6) state_organization_pairings[key]["fillKey"] = String(num);
      else state_organization_pairings[key]["fillKey"] = "defaultFill";
    }

    // Visualization 3
    let disaster_funding_pairings = {
      "$0 Funding": 0,
      "$1 - $10,000 Funding": 0,
    ...
    for (let disaster of disaster_data) {
      ...
      else disaster_funding_pairings["$1,000,000,001+ Funding"] += 1;
    }

    let disaster_fund_pairs = [];
    for (const key of Object.keys(disaster_funding_pairings)) {
      disaster_fund_pairs.push({
        label: key,
        value: disaster_funding_pairings[key]
      });
    }
  ...
  render() {
    return (
      <div class="text-center mt-3">
        ...
        >
          <Tabs.Tab id="Visualization 1" title="Visualization 1">
            {this.state.info_loaded ? (
              <div>
                <h2 className="mt-4 ml-1 mr-1">
                  Frequency of Natural Disasters by State
                </h2>
                <Datamap
                  width={this.state.width > 1200 ? 1200 : this.state.width}
                  height={
                    this.state.width > 1200 ? 800 : (2 / 3) * this.state.width
                  }
                  style={{
                    marginTop:
                      this.state.width > 1200
                        ? "-80px"
                        : String(((-0.1 * 2) / 3) * this.state.width) + "px"
                  }}
                  scope="usa"
                  geographyConfig={{
          ...
          <Tabs.Tab id="Visualization 2" title="Visualization 2">
            {this.state.info_loaded ? (
              <div>
                <h2 className="mt-4 ml-1 mr-1">
                  Frequency of Organizations by State
                </h2>
                <Datamap
                  width={this.state.width > 1200 ? 1200 : this.state.width}
                  height={
          ...
          <Tabs.Tab id="Visualization 3" title="Visualization 3">
            {this.state.info_loaded ? (
              <div>
                <h2 className="mt-4 ml-1 mr-1">
                  Frequency of Natural Disasters by Funding
                </h2>
                <BubbleChart
                  graph={{
                    zoom: 1,
